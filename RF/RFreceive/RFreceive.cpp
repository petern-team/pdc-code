...
RFreceive::RFreceive(long product) {
    pinMode(RECEIVEPIN, INPUT);
    //  irrecv.enableIRIn();              // Start the IR receiver
...
bool RFreceive::checkTransmission() {
...
bool RFreceive::parseTransmission(unsigned int write_array[][10]) {
    int chk_indx;
...
int RFreceive::checkCharSum(int chk_indx, bool *error_found) {
    int checksum = 0;
    int num_chars = 0;
    short char_to_int;
    int data_index;
...
long RFreceive::pow(int base, int power) {
    long ans = 1;

/*
  ...
  Ultrasound sensor sampling code credited to http://arduino.cc/en/Tutorial/Ping?from=Tutorial.UltrasoundSensor

    General operating principles: The sensor board consists of a button, an LED, an RF receiver
  and transmitter. The LED stays solid when the sensor is paused and flashes at twice the sample 
  rate when sampling. Every time the sensor is paused it looks for a DTD to sync with; if it doesn't 
  receive a sync code back, it keeps all data and stays paused, otherwise it dumps data via RF to the 
  DTD. After dumping data, it waits to receive a confirmation code; if no code is received it returns to 
  without reseting data storage, otherwise it resets all the storage variables and stays paused. The computer/ 
  DTD can choose whether or not to send a confirm code, thereby deciding whether or not to erase storage
*/
...
const int TRANSMIT_PIN = 9;          //  These two will stay the same between RF and IR
const int RECEIVE_PIN = 10;          //  implementations
...
button sync_button(0);      // button on interrupt 0 = pin 2
...
//  TCCR0B |= (1 << CS02) | (1 << CS00); // 1024 prescaler - not for timer2
  ...
  index = 0;
  eeprom_time_index = 0;
  eeprom_stor_index = STORAGE_START;    // start the data value storage about halfway
...
// DTDsync is called after the docking station initiates a sync with the sensor board
// 
...
//  Serial.print(address-1); Serial.print(", "); Serial.println(EEPROM.read(address-1));
  ...
  if(address == ID_ADRS+1) {
    ...
    Serial.print("using product id: "); Serial.println(PRODUCT_ID);
    ...
    Serial.print("using sensor: "); Serial.println(SENSOR_TYPE);
    ...
    Serial.print("using sample rate: "); Serial.println(DATA_INTERVAL);
  }
}
